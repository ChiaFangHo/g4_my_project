...
//----------------留言板功能----------------
	...
	function fnsubmit()
	{
		...
		var o_btn=document.createElement("button"); //檢舉按鈕
		...
		var str_name=document.createTextNode((rowCount++)+"樓 "+nametxt.value+": "); // 幾樓和暱稱
		...
		$.ajax({
			url:servletPath(),
			data:{"metChoice":"comment","diaryid":diaryid,"nickname":repCustNickName,"commenttext":repComm},
			type:"POST",
			success:function(data){
				alert("留言已送出");
				...
				$(".box_btn").attr("data-repDiaryID",diaryid);
	...
	function servletPath(){
		...
		var serverEndPoint = "/CommentServlet";
